...
function install_packages ()
{
  ...
  echo "Install EPEL and xmlstarlet"
  amazon-linux-extras install epel -y
  yum -y --enablerepo epel install xmlstarlet

  echo "Install Jenkins"
  wget -O /etc/yum.repos.d/jenkins.repo http://pkg.jenkins-ci.org/redhat-stable/jenkins.repo
  rpm --import https://pkg.jenkins.io/redhat-stable/jenkins.io.key
  yum install -y jenkins java-1.8.0-openjdk-devel git python2-pip nc
  pip install bcrypt
...
EOF
  ...
  wget -q http://127.0.0.1:8080/jnlpJars/jenkins-cli.jar  -O /var/lib/jenkins/jenkins-cli.jar
